    ...
    <div className='marged' >
      ...
      <div className="mt-10" >
          ...
          <div>
            <Link to="/profile" className='d-flex align-items-center'>
              ...
              <div className='ml-1' style={{ transform: 'translateY(-2px)' }}>
                <span className='d-block' style={{color:'white'}}>{currentUser.name}</span>
              </div>
            </Link>
          </div>
        </div>
        ...
        <div className='d-flex justify-content-between align-items-center my-8'>
          ...
          <i className='fa fa-redo' style={{ cursor: 'pointer', marginLeft: '180px' }}
        ...
        <div className='suggestions' >
          {
            userSuggestion && userSuggestion.map(user=>
              <div className='d-flex justify-content-between' style={{color:'white'}} key={user._id}>
              <div>
    
                <Link to={`/profile/${user._id}`} className='d-flex align-items-center'>
                  <img src={user.pic} className='sug-avatar' alt='avatarsug' />&nbsp;
                  <div className='ml-1' style={{ transform: 'translateY(-2px)' }}>
                    <span className='d-block' style={{color:'white'}}>{user.name}</span>
                  </div>
                </Link>
              </div>
              <button style={{ marginLeft: '180px', marginTop: '20px'}} className="sug-followbtn">
                Follow
              </button>
            </div>
            )
